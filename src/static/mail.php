...
foreach ( $_POST as $key => $value ) {
  if ( $value != "" && $key != "project_name" && $key != "admin_email" && $key != "form_subject" && $key != "calculator") {
    ...
    " . ( ($c = !$c) ? '<tr>':'<tr style="background-color: #f8f8f8;">' ) . "
      <td style='padding: 10px; border: #e9e9e9 1px solid;'><b>$text</b></td>
...
if (!empty($_POST['calculator'])) {
  ...
  $mes = "<h1>The client didn`t select anything in the calculator</h1>";
...
try {
  ...
  $mail->Username   = 'loadranswebsender@gmail.com'; // Логин на почте
  $mail->Password   = 'ivalqtfurdajdvre'; // Пароль на почте
  $mail->SMTPSecure = 'ssl';
  $mail->Port       = 465;

  $mail->setFrom('loadrans@gmail.com', 'Moving Quote'); // Адрес самой почты и имя отправителя

  // Получатель письма
  $mail->addAddress('loadrans@gmail.com');
